...
On the same day, we had the layout of our design done and the broke down tasks between frontend and backend which was useful. We are going into the project fully organised and very well prepared.
...
Day two when the real work starts, we setup a Trello board with all the relevant tasks that needed to be done and each member is to assign their name to the tasks they were done, then on completion mark as completed. 
